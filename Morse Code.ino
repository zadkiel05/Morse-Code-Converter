...
void setup() {
  ...
  pinMode(buttonEnd, INPUT); // Signifies the end of a letter or lexicon 
  pinMode(buttonStartEnd, INPUT); // Signifies the end or start of the transmission 
...
void loop() {
  ...
  buttonStartEndState = digitalRead(buttonStartEnd); // will flicker between 1s and 0s depending upon whetehr pin 11 is high or low 
  // Everytime the button thats start and pauses the program is pressed, the program increases 'cont' in one. Function : Tells whether the transmission is ending or starting
  if(buttonStartEndState){
    cont++; // The amount of times the button is pressed
  ...
  if(cont%2==1){
      ...
      if (timeButtonNotPressed % 15000 == 0){
      Serial.print(" ");
      lcd.print(" ");
      lcdPos ++;
      //delay(300);
...
// while the button is pressed the time is count
  while (buttonMorseState){
    buttonMorseState = digitalRead(buttonMorse);  // loop only for when the morse button is pressed 
    tone(buzzer, 800);
  }
  noTone(buzzer);
// finalize the time and store in timeButtonPressed
  timeButtonPressed = millis() - timeStart;  // Time taken to execute the above buzzer morse logic 
